# Development Environment - ANC Platform Terraform Configuration
# Lower cost configuration suitable for development and testing
# Features:
#   - Single-AZ deployment for cost savings
#   - Smaller instance sizes
#   - Spot instances enabled for Lambda
#   - Minimal replication
...
module "lambda" {
  ...
  # Environment variables for Lambda - mapped from other modules
...
# CloudFront CDN (optional for dev)
module "cloudfront" {
...
# Outputs - expose important values for application configuration
output "environment" {
...
output "audio_queue_url" {
  description = "SQS queue URL for audio processing"
  value       = module.sqs.audio_queue_url
}

output "output_queue_url" {
  description = "SQS queue URL for processed audio"
  value       = module.sqs.output_queue_url
}
